...
def fetch_proxies():
    response = requests.get('http://localhost:9097/proxies')
    # 获取原始数组
...
def delay_test():
    try:
        requests.get(f'https://www.google.com/generate_204', timeout=5)
        return True
    except:
        return False
...
def choose_proxy(proxy):
    ...
    # print(f'http://localhost:9097/proxies/{encoded_proxy}')
    data = {
        "name": proxy
    }
    try:
        requests.put(url=f'http://localhost:9097/proxies/{encoded_proxy}', json=data)
        print(f'{proxy} 选中成功')
        return True
    except:
        return False
